    ...
    if(algo == "GEN_QUERY"){
        ...
        for(int para = 0; para < 1; para++){
            ...
            for(int t = 0; t < node_count; t++){
				...
				double temp_err=0;
				for(auto ppr_pair : topkppr ){
					//cout<<ppr_pair.first<<endl;
					auto it=realMap.find(ppr_pair.first);
					if (it == realMap.end()) 
					{ 
						std::cout << "key not found\n"; 
						continue;
					} 
					//cout<<(*it).first<<" "<<abs(ppr_pair.second-(*it).second)<<endl;
					temp_err+=abs(ppr_pair.second-(*it).second);
				}
				err+=temp_err/topkppr.size();
            }
