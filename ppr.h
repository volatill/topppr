    ...
    vector<pair<int, double> > TopPPR(int s, double r_threshold, int k){
        ...
        while(true){   
            ...
            if(candidate_size <= backSizeThreshold || leftCandidateSize >= k - error_num || gap2 < 0.00000001/*|| (r_sum > 0 && r_sum < 0.0000000001)*/){
                ...
                double NDCG = calNDCG(newLeftVec,realList,realMap,s,k);
                //cout << "NDCG: " << NDCG << endl;
...
/*void PowerMethodK2(int iterations, string outputFile, int u, int k){
    ...
    double calNDCG(vector<int> candidates, vector<int> topK, unordered_map<int, double> realMap, int k, int s){
        //unordered_map<int, double> realMap = getRealTopKMap(s, k);

        double correct = 0;
        for(int i = 0; i < topK.size(); i++){
            if(candidates[i] == topK[i])
                correct++;
            else{
        ...
        for(int i = 1; i <= topK.size(); i++){
            //cout<<"Zp:"<<(pow(2, realMap[topK[i-1]]) - 1) / log(i+1) / log(2)<<endl;
            //cout<<realMap[topK[i-1]]<<endl;
            Zp += (pow(2, realMap[topK[i-1]]) - 1) / (log(i+1) / log(2));
        }
        double NDCG = 0;
        for(int i = 1; i <= topK.size(); i++){
            //cout<<"NDCG:"<<(pow(2, realMap[candidates[i-1]]) - 1) / (log(i+1) / log(2))<<endl;
            NDCG += (pow(2, realMap[candidates[i-1]]) - 1) / (log(i+1) / log(2));
        }

        //cout<<"Zp:"<<Zp<<"\tNDCG"<<NDCG<<endl;
        return NDCG / Zp;
    }
